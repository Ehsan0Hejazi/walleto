...
import { getUnCategorizedIncomesAsync } from '../../../redux/user/user.action';
...
function Categorized(props) {
  ...
  useEffect(() => {
    ...
    props.navigation.addListener('focus', () => {
      props.setCurrentTopTab(TopTabs.INCOME_UNCAT);
    });
  }, []);

  useEffect(() => {
    setData(props.unCatIncomes);
  }, [props.unCatIncomes]);
...
const mapStateToProps = state => ({
  unCatIncomes: state.user.unCatIncomes,
});
